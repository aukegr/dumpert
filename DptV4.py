...
def check_if_valid_url(check_url):
    if check_url[:28] == "https://www.dumpert.nl/item/":
        return True
    else:
        return False

def count_items(check_url):
    global last_viewed_item
    global data_loaded
    if check_if_valid_url(check_url) == False:
        return m.set("Geen geldige link")
    check_item = re.sub('.*(?<=item\/)',"",check_url)
    data_loaded = False
    df = load_items(check_item)
...
ttk.Button(root, text='Open dumpstats', command=lambda: open_dumpstats()).pack(ipadx=5, ipady=5, expand=True)
...
#url_input.set('Drop/paste here...')
dl = tk.Entry(root, textvar=url_input)
dl.pack(ipadx=5, ipady=5, expand=True)
dl.drop_target_register(DND_TEXT)
dl.dnd_bind('<<Drop>>', drop)
...
ttk.Button(root, text='Tel', command=lambda check_url=dl.get(): count_items(dl.get())).pack(ipadx=5, ipady=5, expand=True)
ttk.Label(root, textvariable=m).pack(ipadx=5, ipady=5, expand=True)
ttk.Button(root, textvariable=n, command=lambda check_url=dl.get(): open_x_items(dl.get())).pack(ipadx=5, ipady=5, expand=True)
ttk.Button(root, text='Refresh', command=lambda check_url=dl.get(): refresh(dl.get())).pack(ipadx=5, ipady=5, expand=True)
